...
<body>
	...
	<?php 
		$sql="SELECT * FROM tac_obreros WHERE obr_cedula='".$_SESSION["cedula"]."'";
		$rs= mysql_query($sql) or die(mysql_error());
		$row= mysql_fetch_array($rs);

		$sql2="SELECT actividad FROM c_actividades WHERE id_actividad='".$row["c_actividad"]."'";
	...
	<div class="container">
			<div id="main" class="table-responsive">
				<h4 style="display: inline-block">Bienvenido(a): <?php echo $row["obr_nomap"]; ?> </h4> <a href="session/cerrar_sesion.php">Cerrar Sesión</a>
				<table class="table table-striped table-condensed">
				  ...
				  <tbody>
				  	<tr>
					  	<td><center><?php echo $row["obr_cedula"]; ?></center></td>
					  	
					  	<td><center><?php echo $row["obr_cargo"]; ?></center></td>
						...
						<div class="panel panel-default">
							  		<div class="panel-heading">Constancias</div>
							  			<div class="panel-body">
							  				...
							  				echo '<a class="btn btn-success" href="ultima_obreros.php"><span class="glyphicon glyphicon-save"></span> Ultima constancia Generada</a> Fecha: '.$fecha_de.'<br><br>';
							  			} ?>
			...
			<div class="col-md-6">
				<div class="panel panel-default">
					  		<div class="panel-heading">Recibos de Pago</div>
					  			...
					  			<h3><?php mes($q); ?></h3>
					  			<?php 
					  				$cod=explode('-', $row['obr_cod']);
					  				if ($q>date('m')) {
					  					echo '<p class="bg-danger">Aun no se han generado ricibos</p>';
					  				}elseif (isset($q) && $cod[0]=='OS' || $cod[0]=='OF') { ?>
					  				...
					  				if ($q>date('m')) {
					  					echo '<p class="bg-danger">Aun no se han generado ricibos</p>';
					  				}elseif (isset($q) && $cod[0]=='BE') { ?>
					  					<table class="table table-striped">
						  				 	<tr>
						  				 		<td>Recibo de pago</td>
						  				 		<td><a href="recibos/BE.php?p=<?php echo $row["obr_cedula"]; ?>&q=<?php echo $q; ?>" target="_blank"><span class="glyphicon glyphicon-save"></span></a></td>
						  				 	</tr>
